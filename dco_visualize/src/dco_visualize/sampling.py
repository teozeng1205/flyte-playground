...
QUALITY_COLUMNS = ["trip_type", "cabin", "stops", "source", "carrier"]
...
def _build_quality_metrics(
    ...
    return {
        ...
        "top_airport_market_coverage": float(
            airport_coverage_count / max(len(top_airport_markets), 1)
        ),
...
def build_representative_samples_from_parquet(
    ...
    *,
    ...
    quality = {
        ...
        "train": _build_quality_metrics(sampling_frame, viz_sampling_frame[viz_sampling_frame["row_id"].isin(train_row_ids)], config),
    }
    result = {
