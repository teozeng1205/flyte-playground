...
"""Generate metrics table with all dimensions: validatingCarrier, origin country - destination country, and cabin.

Requested columns:
validatingcarrier, country_od, cabin, samplecount, median_totalamount, avg_totalamount,
...
citylocation_path = sys.argv[2] if len(sys.argv) > 2 else "citylocation.csv"
out_path = sys.argv[3] if len(sys.argv) > 3 else "metrics_all.csv"

df = read_metrics_dataframe(path)

# Load city location data to get country codes
citylocation = pd.read_csv(citylocation_path)
# Create mapping from citycode to countrycode
city_to_country = dict(zip(citylocation['citycode'], citylocation['countrycode']))

# Map origin and destination city codes to country codes
df['origin_country'] = df['originCityCode'].map(city_to_country)
df['destination_country'] = df['destinationCityCode'].map(city_to_country)

# Drop rows where country mapping is missing
df = df.dropna(subset=['origin_country', 'destination_country'])

# Create combined country_od column
df['country_od'] = df['origin_country'] + '-' + df['destination_country']
...
group_cols = ['validatingCarrier', 'country_od', 'cabin']
...
rows = []
for (carrier, country_od, cabin), g in df.groupby(group_cols):
    samplecount = len(g)
...
cols = [
    'validatingcarrier','country_od','cabin','samplecount','median_totalamount','avg_totalamount','avg_yq','avg_yr','avg_yqyr',
