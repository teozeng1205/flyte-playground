...
"""Generate per-validatingCarrier and origin country - destination country metrics table.

Requested columns:
validatingcarrier, country_od, samplecount, median_totalamount, avg_totalamount,
...
out_path = sys.argv[3] if len(sys.argv) > 3 else "metrics_country_od.csv"
...
group_cols = ['validatingCarrier', 'country_od']
...
rows = []
for (carrier, country_od), g in df.groupby(group_cols):
    samplecount = len(g)
...
cols = [
    'validatingcarrier','country_od','samplecount','median_totalamount','avg_totalamount','avg_yq','avg_yr','avg_yqyr',
